@if ($quest->deleted_at)
    <form action="{{ route('dashboard.questions.restore', $quest->id) }}" method="post" class="d-inline">
        @method('PATCH')
        @csrf
    ...
    <form action="{{ route('dashboard.questions.destroy', $quest->id) }}" method="post" class="d-inline"
...
@else
    @include('dashboard.questions.edit')
    <form action="{{ route('dashboard.questions.destroy', $quest->id) }}" method="post" class="d-inline"
        onsubmit="return confirm('apakah anda yakin?')">
        @csrf
        <input type="hidden" name="_method" value="delete" />
